            ...
            sh '''#!/bin/bash
				...
				then
                    ...
                    python launch_benchmark.py \
                        ...
                        --batch-size=1 \
                        --socket-id 0 \
                        --in-graph /tf_dataset/pre-trained-models/resnet50/fp32/freezed_resnet50.pb \
                ...
                then
                    ...
                    python ${WORKSPACE}/tensorflow-intelai-tools/api/quantize_model.py \
                        --model resnet50 \
                        --in_graph /tf_dataset/pre-trained-models/resnet50/fp32/freezed_resnet50.pb \
                        --out_graph ${WORKSPACE}/resnet50-quantize-${HOSTNAME}.pb \
                    ...
                    benchmark_log_file="${WORKSPACE}/${log}"
                    python launch_benchmark.py \
                        --in-graph ${WORKSPACE}/resnet50-quantize-${HOSTNAME}.pb \
                        --model-name resnet50 \
                        --framework tensorflow \
                        --precision int8 \
                        --mode inference \
