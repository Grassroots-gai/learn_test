...
function set_environment {
	...
	export PYTHONPATH=/home/tensorflow/limengfx/pytorch/ssd_resnet34/dl_framework-pytorch_aten_integration/build/
...
function generate_core {
    ...
    do
        ...
        printf " OMP_NUM_THREADS=${real_cores_per_instance} numactl --localalloc --physcpubind ${cpu_array[i]} \
                 python train.py -a ssd_r34 --dummy 1 -b ${batch_size} --log $i \
...
function collect_logs {
    ...
    throughput=$(grep '100it' ${log_dir}/rcpi* |grep 'it/s' |sed -e 's/.*log//;s/.*,//;s/[^0-9.]//g' |awk '
    BEGIN {
        sum = 0;
    }
    {
