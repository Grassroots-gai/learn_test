...
function generate_core {
    ...
    do
        ...
        printf " OMP_NUM_THREADS=${real_cores_per_instance} numactl --localalloc --physcpubind ${cpu_array[i]} \
                python run_tf_benchmark.py -c ${in_graph} --layers 256,256,128,64 --num_factors 64 \
                -a ${real_cores_per_instance} -e 1 -p 1 -b ${batch_size} -mts 1000 \
                -v -f --synth --cpu --inference -n ${real_cores_per_instance} \
...
function collect_logs {
    latency=$(grep 'Latency:' ${log_dir}/rcpi${cores_per_instance}* |sed -e 's/.*log//;s/[^0-9.]//g;s/.$//' |awk  '
    BEGIN {
        sum = 0;
        i = 0;
    }
    ...
    throughput=$(grep 'Throughput:' ${log_dir}/rcpi* |sed -e 's/.*log//;s/[^0-9.]//g;s/.$//' |awk '
    BEGIN {
        sum = 0;
    }
    {
