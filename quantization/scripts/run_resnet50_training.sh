...
function generate_core {
    ...
    do
        ...
        printf " OMP_NUM_THREADS=${real_cores_per_instance} numactl --localalloc --physcpubind=${cpu_array[i]} \
                ...
                --mkl=True --forward_only=False --num_batches=100 --kmp_blocktime=0 --num_warmup_batches=30 \
                --num_inter_threads=1 --distortions=False --optimizer=sgd \
                --batch_size=${batch_size} --num_intra_threads=${real_cores_per_instance} \
                --data_format=NCHW --num_omp_threads=${real_cores_per_instance} --model=${model} \
