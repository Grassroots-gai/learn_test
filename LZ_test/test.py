...
class LoggerHook(tf.estimator.SessionRunHook):
    ...
    def before_run(self, run_context):
        ...
        tf.logging.info("iteration %d", self._step)
    ...
    def end(self, run_context):
        tf.logging.info("Steps: %d ", self._step)
        latency = np.mean(self.duration_list)
        tf.logging.info("Latency: %.5f s", latency)
        tf.logging.info("Throughput: %.5f", (self.bs / latency))
...
class Model(object):
    ...
    def train(self):
        ...
        for _ in range(self.conf.train_epochs // self.conf.epochs_per_eval):
            ...
            if self.conf.validation_id != -1:
                ...
                start = time.time()
                classifier.evaluate(input_fn=input_fn_eval, steps=5)
                end = time.time()
                evaluate_duration = end - start
                tf.logging.info('Evaluation duration is %f for %d steps', evaluate_duration, 5)
                print('Throughput is %.2f examples/s' % (self.conf.batch_size_eval * 5 / evaluate_duration))
    ...
    classifier = tf.estimator.Estimator(
        model_fn=self._model_fn,
        model_dir=self.conf.model_dir, config=config)
    ...
    preds = classifier.predict(
        ...
        hooks=[LoggerHook(self.conf.batch_size)])
