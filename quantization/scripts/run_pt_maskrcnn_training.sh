    ...
    do
        ...
        printf " OMP_NUM_THREADS=${real_cores_per_instance} numactl --localalloc --physcpubind ${cpu_array[i]} \
                  python tools/train_mlperf.py \
                    ...
                    --iters 300 \
                    --log $i \
                    SOLVER.IMS_PER_BATCH 1 TEST.IMS_PER_BATCH 1 SOLVER.MAX_ITER 720000 SOLVER.STEPS "\(480000, 640000\)" SOLVER.BASE_LR 0.000000001 MODEL.DEVICE cpu \
...
function collect_logs {
    ...
    throughput=$(grep 'MLPERF METRIC THROUGHPUT per CPU=' ${log_dir}/rcpi* |sed -e 's/.*log//;s/.*=//;s/iter.*//' |awk '
    BEGIN {
        sum = 0;
    }
    {
