        ...
        node(label) {
            ...
            try {
                ...
                stage("Clone repository $label") {
                    ...
                    //checkout([$class: 'GitSCM', branches: [[name: "*/$PRIVATE_TENSORFLOW_REPO_BRANCH"]], doGenerateSubmoduleConfigurations: false, extensions: [[ $class: 'RelativeTargetDirectory', relativeTargetDir: 'private-tensorflow' ]], submoduleCfg: [], userRemoteConfigs: [ [credentialsId: "$GIT_CREDENTIAL_LAB", url: "$GIT_URL"]]])
        ...
        relativeTargetDir: 'models']], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'lab_tfbot', url: 'https://github.com/tensorflow/models.git']]])
                    ...
                    checkout([
                        ...
                        submoduleCfg: [], userRemoteConfigs: [
                            [credentialsId: "$GIT_CREDENTIAL_LAB", name: "$GIT_NAME",
            ...
            userRemoteConfigs: [[credentialsId: "$GIT_CREDENTIAL", 
                ...
                stage("Build on $label") {
                    ...
                    if (RUN_TYPE == "mklml") {
                        echo "mklml"
                        summaryTitle="Tensorflow MKL-ML convergene test summary"
                    }
                    else {
                        echo "mkldnn"
                        summaryTitle="Tensorflow MKL-DNN convergence test summary"
                    }
                    echo "NODE_NAME = ${env.NODE_NAME}"
                    echo "Github Pull ID = ${ghprbPullId}"

                    withEnv(["date=$date","summaryTitle=$summaryTitle","gitCommit=$gitCommit", "model_name=$model_name","os_version=$os_version", "nodeName=${env.NODE_NAME}", "repoBranch=${PRIVATE_TENSORFLOW_REPO_BRANCH}", "label=${label}", "runType=${RUN_TYPE}","gitPullID=${ghprbPullId}"]) {
                        sh '''#!/bin/bash -x
                ...
                stage("Run convergence test $label") {
                    sh '''#!/bin/bash -x
                ...
                withEnv(["runType=$RUN_TYPE", "label=$label"]) {
