...
SSDMobilenet_URL='https://gitlab.devtools.intel.com/TensorFlow/Shared/tensorflow-models.git'
SSDMobilenet_BRANCH='hfei3/int8-ssdmobilenet'
SSDMobilenet_TARGETDIR='tensorflow-models'
...
// TODO: list the Q4 fp32 models here separated by comma
MODELS="FaceNet,MTCC,ResNet101,DenseNet169"
if ('MODELS' in params) {
...
SUMMARY_TITLE = 'private-tensorflow nightly benchmark Q4 FP32 models'
...
def runBenchmark(String docker_image, String dataset_location, Boolean single_socket, String servername, String models, String modes, String git_name, Boolean download_wheel) {
    ...
    docker.image("$docker_image").inside("--env \"http_proxy=${http_proxy}\" \
            ...
            chmod 775 /workspace/cje-tf/scripts/run_benchmark_q4_fp32_models.sh
            /workspace/cje-tf/scripts/run_benchmark_q4_fp32_models.sh
            '''
    }
}
        ...
        modes.each { mode ->
            ...
            withEnv(["current_model=$model","current_mode=$mode", "single_socket=${single_socket}", "servername=${SERVERNAME}"]) {
                ...
                if [ -f "$WORKSPACE/benchmark_${current_model}_${current_mode}_${servername}_latency.log" -o -f "$WORKSPACE/benchmark_${current_model}_${current_mode}_${servername}_throughput.log" ]; then
                    ...
                    chmod 775 $WORKSPACE/cje-tf/scripts/collect_logs_q4_fp32_models.sh
                    $WORKSPACE/cje-tf/scripts/collect_logs_q4_fp32_models.sh --model=${current_model} --mode=${current_mode} --single_socket=${single_socket}
                fi
    ...
    try {
        ...
        stage('Checkout') {
            ...
            checkout([$class                           : 'GitSCM',
                      ...
                      doGenerateSubmoduleConfigurations: false,
                      extensions                       : [[$class           : 'RelativeTargetDirectory',
                                                           relativeTargetDir: 'tensorflow-FaceNet']],
                      submoduleCfg                     : [],
                      userRemoteConfigs                : [[credentialsId: "$GIT_CREDENTIAL",
                                                           url          : 'https://gitlab.devtools.intel.com/TensorFlow/Shared/tensorflow-FaceNet.git']]])
            ...
            checkout([$class                           : 'GitSCM',
                      ...
                      doGenerateSubmoduleConfigurations: false,
                      extensions                       : [[$class           : 'RelativeTargetDirectory',
                                                           relativeTargetDir: 'tensorflow-MTCC']],
                      submoduleCfg                     : [],
                      userRemoteConfigs                : [[credentialsId: "$GIT_CREDENTIAL_LAB",
                                                           url          : 'https://gitlab.devtools.intel.com/TensorFlow/Shared/tensorflow-MTCC.git']]])
	    ...
	    // DenseNet169 
            checkout([$class                           : 'GitSCM',
                      ...
                      doGenerateSubmoduleConfigurations: false,
                      extensions                       : [[$class           : 'RelativeTargetDirectory',
                                                           relativeTargetDir: 'tensorflow-DenseNet169']],
                      submoduleCfg                     : [],
                      userRemoteConfigs                : [[credentialsId: "$GIT_CREDENTIAL",
                                                           url          : 'https://github.com/NervanaSystems/tensorflow-Desnet169.git']]])


    	    // SSDMobilenet
            checkout([$class                           : 'GitSCM',
                      branches                         : [[name: SSDMobilenet_BRANCH]],
                      browser                          : [$class: 'AssemblaWeb', repoUrl: ''],
                      doGenerateSubmoduleConfigurations: false,
                      extensions                       : [[$class           : 'RelativeTargetDirectory',
                                                           relativeTargetDir: SSDMobilenet_TARGETDIR]],
                      submoduleCfg                     : [],
                      userRemoteConfigs                : [[credentialsId: "$GIT_CREDENTIAL_LAB",
                                                           url          : SSDMobilenet_URL]]])
        ...
        if (POST_TO_DASHBOARD) {
            ...
            stage('Post to AVUS dashboard') {
                ...
                if (TENSORFLOW_BRANCH == 'master')
                    POSTING_MODELS = [ "FaceNet", "MTCC", "DenseNet169" ]
                else
                    POSTING_MODELS = [ "ResNet101", "SSDMobilenet" ]
                ...
                LOGS_TYPE = [ "accuracy"]
                if (TENSORFLOW_BRANCH == 'master')
                    POSTING_MODELS = [ "DenseNet169" ]
                else
                    POSTING_MODELS = [ "ResNet101" ]
                postAll2AIBTdashboard(RUNTYPE, FRAMEWORK, FRONTEND, TARGET_DASHBOARD, LOGS_DIR, LOGS_TYPE, POSTING_MODELS, DATATYPE)
