    ...
    do
        ...
        printf " OMP_NUM_THREADS=${real_cores_per_instance} numactl --localalloc --physcpubind ${cpu_array[i]} \
            python launch_benchmark.py \
                ...
                --num-intra-threads=${real_cores_per_instance} \
                --num-inter-threads=1 \
                --in-graph=${in_graph} \
                --benchmark-only \
...
function collect_logs {
    if [ "${precision}" == "fp32" ];then
        ...
        latency=$(grep 'Latency:' ${log_dir}/rcpi${cores_per_instance}* |sed -e 's/.*log//;s/.*mean= *//;s/ .*//' |awk  '
        BEGIN {
            sum = 0;
            i = 0;
        }
        ...
        throughput=$(grep 'Latency:' ${log_dir}/rcpi* |sed -e 's/.*log//;s/.*mean= *//;s/ .*//' |awk -v batch_size=$batch_size '
        BEGIN {
            sum = 0;
        }
        {
            sum = sum + batch_size * 1000 / $1;
        }
        END {
            printf("%.2f", sum);
        }')
        ...
        latency=$(grep 'Time spent per BATCH:' ${log_dir}/rcpi${cores_per_instance}* |sed -e 's/.*log//;s/[^0-9.]//g' |awk  '
        BEGIN {
            sum = 0;
            i = 0;
        }
        ...
        throughput=$(grep 'Total samples/sec:' ${log_dir}/rcpi* |sed -e 's/.*log//;s/[^0-9.]//g' |awk -v batch_size=$batch_size '
        BEGIN {
            sum = 0;
        }
        {
