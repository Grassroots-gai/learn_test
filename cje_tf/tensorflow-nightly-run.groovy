...
def createParallelJobs() {
    ...
    parallel_jobs["Unit_Test"] = {
        ...
        if('Unit Test' in task) {
            // eigen
            stage("Eigen Test") {
                eigen_job_name = 'Nightly-Private-Tensorflow-Eigen'
            ...
            // unit test
            stage("Unit Test") {
                unit_job_name = 'Nightly-Private-Tensorflow-Unittest'
    ...
    parallel_jobs["Wheel_Build"] = {

        // build wheel
        if('Wheel Build' in task) {
            stage("Wheel Build") {
                tensorflow_job_name = 'private-tensorflow-build-trigger'
                
                def build_wheel = build job: tensorflow_job_name, propagate: false
                
                task_status = build_wheel.result
                task_number = build_wheel.number
    ...
    parallel_jobs["Container_Build"] = {
        ...
        container_jobs_run["Container QA"] = {

            def container_qa_jobs_run = [:]
            container_qa_jobs = [ 'Intel-Models-Benchmark-fp32-py3-Trigger','Intel-Models-Benchmark-fp32-Trigger','Intel-Models-Benchmark-int8-py3-Trigger','Intel-Models-Benchmark-int8-Trigger' ]
            container_qa_jobs.each {benchmark_job_name ->
                if(benchmark_job_name in job_list) {
                    container_qa_jobs_run["${benchmark_job_name}"] = {
                        catchError {
                            stage("${benchmark_job_name}") {
                                def benchmark_container = build job: benchmark_job_name, propagate: false
                            
                                task_status = benchmark_container.result
                                task_number = benchmark_container.number
                                ...
                                if( benchmark_container.result != 'FAILURE' ) {
                                    catchError {
                                        copyArtifacts(
                                                projectName: benchmark_job_name,
                                                selector: specific("${benchmark_container.getNumber()}"),
                                                filter: '**/*',
                                                fingerprintArtifacts: true,
            ...
            if('Container Build' in task) {
                stage("Container Build QA") {
                    container_job_name = 'Private-TensorFlow-Nightly-Container-Master-MKLDNN-QA'
                
                    retry(retry_num) {
                        def container_qa_build = build job: container_job_name, propagate: false

                        task_status = container_qa_build.result
                        task_number = container_qa_build.number

                        if(task_status == "FAILURE") {
                            sh '''
                                echo "Container QA Build got FAILED!!!"
                                exit 1
                            '''
                        }
                    }
                    ...
                    if('Benchmark' in task && task_status == "SUCCESS") {
                        parallel container_qa_jobs_run
                    }
                }
            }else {
        ...
        container_jobs = [ 'Private-TensorFlow-Benchmark-Q2-Py3-Master','Private-TensorFlow-Benchmark-Q2-Py3-Master-CLX','Private-TensorFlow-Benchmark-Q3-FP32-Models','Private-TensorFlow-Benchmark-Q3-FP32-Models-CLX','Private-TensorFlow-Benchmark-Q3-Int8-Models','Private-TensorFlow-Benchmark-Q3-Int8-Models-CLX','Private-TensorFlow-Benchmark-Q4-FP32-Models','Private-TensorFlow-Benchmark-Q4-FP32-Models-CLX','Private-TensorFlow-Benchmark-Q4-Int8-Models','Private-TensorFlow-Benchmark-Q4-Int8-Models-CLX','tensorflow-skylake-benchmark-nightly','Private-TensorFlow-Benchmark-Q2-Py2-Master', 'Private-TensorFlow-Benchmark-Q2-Py2-Master-CLX' ]
        ...
        if('Container Build' in task) {
            ...
            stage("Container Build") {
                container_job_name = 'Private-TensorFlow-Nightly-Container-Master-MKLDNN'
    ...
    if('Check Time Bombs' in task) {
        parallel_jobs["Check_Time_Bombs"] = {
            ...
            stage("Check Time Bombs") {
                ...
                tool_job_name = "tool-check-time-bombs"
        ...
        dir( WORKSPACE ) {
            ...
            withEnv(["benchmark_log=${benchmark_log}","Post2Heims=${Post2Heims}","test_start_date=${test_start_date}"]) {
                sh '''#!/bin/bash 
                    ...
                    if [ "${Post2Heims}" != "false" ];then 
                        python ./cje-tf/scripts/post2heims.py -p ${benchmark_log} -t ${test_start_date} -b master || true
                    fi
                '''
            }
            ...
            withEnv(["send_email=${send_email}","task=${task}","test_start_date=${test_start_date}"]) {
                sh '''
                    ...
                    # detail
                    source ${WORKSPACE}/cje-tf/scripts/generate_nightly_report.sh "V1" "all"
                    mv tensorflow_nightly_report.html detail.html
                    curl -X POST -H "Content-Type: application/html" -d @${WORKSPACE}/detail.html http://heims.sh.intel.com/api/storeFile/tensorflow/nightly:V1/${test_start_date}-detail.html/false
                    
                    source ${WORKSPACE}/cje-tf/scripts/generate_nightly_report.sh "V1"
                    mv tensorflow_nightly_report.html email.html
                    if [ "${send_email}" != "false" ];then 
                        curl -X POST -H "Content-Type: application/html" -d @${WORKSPACE}/email.html http://heims.sh.intel.com/api/storeFile/tensorflow/nightly:V1/${test_start_date}.html/true
                    fi 
                '''
            }
    ...
    try {
        ...
        if('TensorFlow Update' in task) {
            stage("Tensorflow Update") {
                tensorflow_job_name = 'tensorflow-update-repositories'
        ...
        stage("Generate TF Info") {
            ...
            dir("${WORKSPACE}/tensorflow-tensorflow") {
                ...
                sh '''
                    ...
                    echo "tensorflow_branch=master" >> ${WORKSPACE}/tensorflow_info.log
