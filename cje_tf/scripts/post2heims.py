...
def GetArgumentParser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="tensorflow night test version."
    )
...
def main():
    ...
    jenkins_url = "https://aipg-jenkins-tf.intel.com/job/"
    res_dict = {}
    if args.log_path =="" or args.branch == "":
        print("pls input the log you want to post!\n e.g: python post2hemis.py -p log_path -b branch -t test_time -v tf_version")
        exit(0)
    else:
#         print(args.log_path)
                        ...
                        for line in model_type:
                            ...
                            if m == line.split(';')[0]:
                                if line.split(';')[4] == 'Latency':
                                    have_lat =1
                                    test_job_name = line.split(';')[-2]
                                    build_num = line.split(';')[-1]
                                    log_name = line.split(';')[-3]
                                    link_addr = jenkins_url + test_job_name + "/"  + build_num + "/artifact/" + log_name
                                    data_type = line.split(';')[3]
                                    bs = line.split(';')[5]
                                    value = 'None'
                                    if line.split(';')[6] != '':
                                        value = line.split(';')[6]
                                    latency_dict[data_type] = {'bs':bs, 'value':value, 'link':link_addr, "tolerance":""}
                                    ...
                                    if line.split(';')[6] != '':
                                        value = line.split(';')[6]
                                    acc_dict[data_type] = {'bs':bs, 'value':value, 'link':link_addr, "tolerance":""}
                                    ...
                                    if line.split(';')[6] != '':
                                        value = line.split(';')[6]
                                    thrpt_dict[data_type] = {'bs':bs, 'value':value, 'link':link_addr, "tolerance":""}
                        if have_lat:
