...
if [[ ${model_name} == "ssd-mobilenet" ]]; then
  ...
  elif [[ ${precision} == "int8" ]] && [[ ${accuracy} == "false" ]]; then
      in_data_location="/tf_dataset/sh_data/COCO2017/val2017/"
  elif [[ ${precision} == "int8" ]] && [[ ${accuracy} == "true" ]]; then
      cp /tf_dataset/sh_data/ssd_mobilenet/v1.5/coco_val.record ${WORKSPACE}/
      in_data_location="${WORKSPACE}/coco_val.record"
  fi
fi
...
case ${model_name} in
  ...
  "mobilenet_v1")
      ...
      if [[ ${log_dir} == *latency* ]]; then
          if [[ ${precision} == "fp32" ]]; then
              latency=$(grep 'Latency ms/step' ${log_dir}_*.log |sed 's/.* =//;s/[^0-9.]//g'| awk 'BEGIN{sum=0}{sum+=$1}END{printf("%.4f\n",sum/NR)}')
          elif [[ ${precision} == "int8" ]]; then
              latency=$(grep -a "steps = 500" ${log_dir}_*.log | sed -e "s/.*, //;s/ images\/sec//" | awk 'BEGIN{sum=0}{sum+=1000/$1}END{printf("%.4f\n",sum/NR)}')
          fi
      elif [[ ${log_dir} == *throughput* ]]; then
          if [[ ${precision} == "fp32" ]]; then
              throughput=$(grep 'Total images/sec' ${log_dir}_*.log |sed 's/.* =//;s/[^0-9.]//g'| awk '{sum+=$1} END {print sum}')
          elif [[ ${precision} == "int8" ]]; then
              throughput=$(grep -a "steps = 500" ${log_dir}_*.log | sed -e "s/.*, //;s/ images\/sec//" | awk 'BEGIN{sum=0}{sum+=$1}END{printf("%.4f\n",sum)}')
          fi
      else
  ...
  "ssd-mobilenet")
      ...
      if [[ ${log_dir} == *latency* ]]; then
          if [[ ${precision} == "fp32" ]]; then
              latency=$(grep 'Latency: ' ${log_dir}_*.log  |sed -e "s/.*mean= //;s/,.*//" | awk 'BEGIN{sum=0}{sum+=$1}END{printf("%.4f\n",sum/NR)}')
          elif [[ ${precision} == "int8" ]]; then
              latency=$(grep 'Avg. Duration per Step:' ${log_dir}_*.log  |sed 's/.*://' | awk 'BEGIN{sum=0}{sum+=$1}END{printf("%.4f\n",sum*1000/NR)}')
          fi
      elif [[ ${log_dir} == *throughput* ]]; then
          if [[ ${precision} == "fp32" ]]; then
              throughput=$(grep 'Latency: ' ${log_dir}_*.log |sed -e "s/.*mean= //;s/,.*//" | awk 'BEGIN{sum=0}{sum+=(1000/$1)}END{printf("%.4f\n",sum)}')
          elif [[ ${precision} == "int8" ]]; then
              throughput=$(grep 'Avg. Duration per Step:' ${log_dir}_*.log  |sed -e 's/.*://' |awk 'BEGIN{sum=0}{sum+=(1/$1)}END{printf("%.4f\n",sum/2)}')
          fi
      else
          accuracy=$(grep 'Average Precision' ${log_dir}.log |head -1 |sed -e 's/.*= //')
      fi
  ...
  "ssd-resnet34")
      ...
      if [[ ${log_dir} == *latency* ]]; then
          ...
          throughput=$(grep 'Total samples/sec:' ${log_dir}_*.log |sed 's/.*://;s/[^0-9.]//g' | awk 'BEGIN{sum=0}{sum+=$1}END{printf("%.4f\n",sum)}')
      else
