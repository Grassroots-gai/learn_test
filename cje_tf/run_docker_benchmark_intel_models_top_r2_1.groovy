...
model_List_fp32 = ['inceptionv3','mlperf_gnmt','resnet101','resnet50','resnet50v1_5','ssd-mobilenet','bert','ssd-resnet34','rfcn','wide_deep','wide_deep_large_ds','densenet169','inceptionv4','bert_official']
...
if (params.DOWNLOAD_WHEEL){
    ...
    DOWNLOAD_WHEEL=params.DOWNLOAD_WHEEL
}
    ...
    def MODELS_CONFIG =
        '''{
            ...
            { 
                ...
                "in-graph-int8":        "pre-trained-models/mobilenet_v1/int8/final_int8_mkl_depthwise_perchannel_Relu6.pb",
                "checkpoint":           "pre-trained-models/mobilenet_v1/fp32",
                "model-sourcedir":      "SSD.TensorFlow",
                "batchsize_accuracy":   "100",
                "batchsize_throughput": "112",
                "model_arg_int8":       "warmup-steps=100 steps=500",
                "num_inter_threads":    "1",
                "num_intra_threads":    "28",
            },
            "ncf":
            ...
            "resnet50" :
            {
                ...
                "batchsize_throughput": "128",
                "batchsize_accuracy":   "100",
                "model_arg_int8":            "steps=1500"
            },     
            "resnet50v1_5" :
            {
                "data-location":        "dataset/TF_Imagenet_FullData",
                "in-graph-fp32":        "pre-trained-models/resnet50v1_5/fp32/resnet50_v1.pb",
                "in-graph-int8":        "pre-trained-models/resnet50_v1_5/int8/freezed_s8s8_resnetv1.5.pb",
                "batchsize_throughput": "128",
                "batchsize_accuracy":   "100"
            },     
            "resnet101" :
        ...
        models.each { model ->
            ...
            modes.each { mode ->
                ...
                withEnv(["current_model=$model","current_mode=$mode", "current_type=$datatype"]) {
                    ...
                    sh '''#!/bin/bash -x
                        chmod 775 $WORKSPACE/cje-tf/scripts/collect_logs_intel_models_r2_1.sh
                        $WORKSPACE/cje-tf/scripts/collect_logs_intel_models_r2_1.sh --model=${current_model} --mode=${current_mode} --datatype=${current_type}                
                    '''
                }
            }
        }
