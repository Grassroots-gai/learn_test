...
def createParallelJobs() {
    ...
    parallel_jobs["Unit_Test"] = {
        ...
        if('Unit Test' in task) {
            // eigen
            stage("Eigen Test") {
                eigen_job_name = 'Nightly-Private-Tensorflow-Eigen-V2'
            ...
            // unit test
            stage("Unit Test") {
                unit_job_name = 'Nightly-Private-Tensorflow-Unittest-V2'
    ...
    parallel_jobs["Container_Build"] = {
        ...
        container_jobs = [ 'Intel-Models-V2-Benchmark-py3-Trigger' , 'Tensorflow-Benchmark-official-v2']
        ...
        if('Container Build' in task) {
            ...
            stage("Container Build") {
                container_job_name = 'Private-TensorFlow-Nightly-Container-Master-MKLDNN-TF-v2-new'
    ...
    if('Check Time Bombs' in task) {
        parallel_jobs["Check_Time_Bombs"] = {
            ...
            stage("Check Time Bombs") {
                ...
                tool_job_name = "tool-check-time-bombs-v2"
        ...
        dir( WORKSPACE ) {
            ...
            withEnv(["benchmark_log=${benchmark_log}","Post2Heims=${Post2Heims}","test_start_date=${test_start_date}"]) {
                sh '''#!/bin/bash 
                    ...
                    if [ "${Post2Heims}" != "false" ];then 
                        python ./cje-tf/scripts/post2heims.py -p ${benchmark_log} -t ${test_start_date} -b "master" -v "v2.0" || true
                    fi
                '''
            }
            ...
            withEnv(["send_email=${send_email}","task=${task}","test_start_date=${test_start_date}"]) {
                sh '''
                    ...
                    # detail
                    source ${WORKSPACE}/cje-tf/scripts/generate_nightly_report.sh "V2" "all"
                    mv tensorflow_nightly_report.html detail.html
                    curl -X POST -H "Content-Type: application/html" -d @${WORKSPACE}/detail.html http://heims.sh.intel.com/api/storeFile/tensorflow/nightly:V2/${test_start_date}-detail.html/false
                    
                    source ${WORKSPACE}/cje-tf/scripts/generate_nightly_report.sh "V2"
                    mv tensorflow_nightly_report.html email.html
                    if [ "${send_email}" != "false" ];then 
                        curl -X POST -H "Content-Type: application/html" -d @${WORKSPACE}/detail.html http://heims.sh.intel.com/api/storeFile/tensorflow/nightly:V2/${test_start_date}.html/true
                    fi 
                '''
            }
