...
function generate_core {
    ...
    do
        ...
        printf " ./mpi.sh ${real_cores_per_instance} $[ ${real_cores_per_instance} - 1 ] ${batch_size} \
...
function collect_logs {
    latency=$(grep 'Latency ms/step =' ${log_dir}/rcpi${cores_per_instance}* |sed -e 's/.*log//;s/[^0-9.]//g;s/.$//' |awk  '
    BEGIN {
        sum = 0;
        i = 0;
    }
    ...
    throughput=$(grep 'samples/sec' ${log_dir}/rcpi* |grep -v 'Throughput' |sed -e 's/.*log//;s/[^0-9.]//g;s/.$//' |awk '
    BEGIN {
        sum = 0;
    }
    {
