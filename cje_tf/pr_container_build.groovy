...
static final String pr_branch = params.get('PR_NUMBER', '')
...
static final String docker_registry_namespace = params.get('IMAGE_NAMESPACE', 'amr-registry.caas.intel.com/aipg-tf/pr')
static final String docker_build_version = params.get('BUILD_VERSION', '')
if (docker_build_version == '') {
    if (docker_tag == 'private-tf') {
        docker_build_version="PR${pr_branch}${build_type_tag}${tf_version_tag}" // To match old PR container name for existing Jenkins jobs
    }
    else {
        docker_build_version="${docker_tag}-PR${pr_branch}${build_type_tag}${tf_version_tag}"
    }    
}
...
node(node_label) {    
    ...
    try {
        ...
        stage('Check Docker') {
            ...
            docker_build_version="PR${pr_branch}-${sha}${build_type_tag}${tf_version_tag}" // To match old PR container name for existing Jenkins jobs
