...
# Tensorflow tag depending upon the python version to pass to the dockerfile
ROOT_CONTAINER_TAG=$(to_lower ${ROOT_CONTAINER_TAG:-latest})
...
# Add the proxy info
#PROXY_INFO="http://proxy-us.intel.com:911"
#NO_PROXY_INFO="localhost,127.0.0.1,intel.com,.intel.com,10.0.0.0/8,172.168.0.0/16,192.168.0.0/16,.internal"
...
echo "TF_DOCKER_BUILD_ARGS=${TF_DOCKER_BUILD_ARGS[@]}"
#"${DOCKER_BINARY}" build --pull -t "${IMG}" "${TF_DOCKER_BUILD_ARGS[@]}" --no-cache -f "${DOCKERFILE}" "${TMP_DIR}"
...
echo "  Log file is at: ${DOCKER_RUN_LOG}"
  ...
  echo ""
  RUN_CMD="${DOCKER_BINARY} run --rm -d -p ${CONTAINER_PORT}:${CONTAINER_PORT} -v ${TMP_DIR}/notebooks:/root/notebooks ${IMG} tail -f /dev/null 2>&1 > ${DOCKER_RUN_LOG}"
  ...
  while [[ -z ${CONTAINER_ID} ]]; do
    ...
    #CONTAINER_ID=$("${DOCKER_BINARY}" images --format "{{.ID}}" ${IMG})
  done
  ...
  echo ""
    ...
    # Commenting out wget tests
    #if wget --no-proxy -qO- "http://127.0.0.1:${CONTAINER_PORT}/tree" &> /dev/null
    #then
    #  echo "  PASS: wget tree"
    #else
    ...
    #  if wget --no-proxy -qO- "${NB_URL}" -o "${TMP_DIR}/${NB_BASENAME}" &> /dev/null
    #  then
    ...
    #done
    #Adding new test
    TEST_CMD=$(docker exec ${CONTAINER_ID} bash -c "python -c 'from tensorflow.python import pywrap_tensorflow; print(pywrap_tensorflow.IsMklEnabled())'")
    echo "Running test command: ${TEST_CMD}" 
    if [ "${TEST_CMD}" = "True" ] ; then
        echo "PASS: MKL enabled test"
    else
        mark_check_failed  "FAIL: MKL enabled test"
    fi
  #fi
